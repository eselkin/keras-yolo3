...
from keras.layers import Conv2D, Add, ZeroPadding2D, UpSampling2D, Concatenate, MaxPooling2D, LSTM, TimeDistributed, Dense, Lambda, Flatten
...
def DarknetConv2D(*args, **kwargs):
    ...
    conv2d = Conv2D(*args, **darknet_conv_kwargs)
    return conv2d
...
def squeeze_middle2axes_operator(x4d):
    ...
    x3d = tf.reshape(x4d, [shape[1], shape[2], shape[3]])
    return x3d
...
def squeeze_middle2axes_shape(x4d_shape):
    ...
    output_shape = (in_rows, in_cols, in_filters)
    return output_shape


def spread_middle2axes_operator(x3d):
    shape = tf.shape(x3d)  # get dynamic tensor shape
    x3d = tf.reshape(x3d, [32, shape[0], shape[1], shape[2]])
    return x3d


def spread_middle2axes_shape(x3d_shape):
    in_rows, in_cols, in_filters = x3d_shape
    output_shape = (32, in_rows, in_cols, in_filters)
    return output_shape
...
def darknet_body(x):
    ...
    x = TimeDistributed(LSTM(32, return_sequences=True, stateful=True, batch_input_shape=(
        xshape[0], xshape[1], xshape[2])), batch_input_shape=(xshape[0], xshape[1], xshape[2], xshape[3]))(x)
    #x = Lambda( spread_middle2axes_operator, output_shape = spread_middle2axes_shape )( x )
    print("SHAPE XXX:", x.shape)
...
def make_last_layers(x, num_filters, out_filters, i):
...
def yolo_body(inputs, num_anchors, num_classes):
    ...
    x, y1 = make_last_layers(
        darknet.output, 512, num_anchors*(num_classes+5), 1)
